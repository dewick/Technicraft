...
public class ConfigurationNode {
  ...
  /**
   * Gets all of the cofiguration values within the Node as a key value pair,
   * with the key being the full path and the value being the Object that is at
   * the path.
   * 
   * @return A map of key value pairs with the path as the key and the object as
   *         the value
   */
   ...
   * 
   * @param node
   *          The map for that node of the tree
   * @return The fully pathed map for that point in the tree, with the path as
   *         the key
   */
   ...
   * could potentially return a default value (not yet implemented) as defined
   * by a plugin, if this is a plugin-tied configuration.
   * 
   * @param path
